...
//认证中心	
	...
	function autonymUpda(){
		...
		$info=$this->upload('idcard',1);
		

		if ($info) {
				foreach ($info as $i => $in) {	 		     	    
		 		$data['idcard_img']=$in['savename'];				
	 		}
			$userinfo->where(array('uid'=>$this->_session('user_uid')))->save($data);			
		 }

		
		 if (count($this->_post('idcard_img'))<1) {
		 	$this->error('营业执照图片未上传','__ROOT__/Center/approve/autonym.html');
		 	exit;
		 }
